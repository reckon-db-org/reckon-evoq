...
# version src/reckon_evoq.app.src happened to name.
#
...
# unreviewed. That mattered especially here, because reckon-evoq's gate is new and
...
RECKON_EVOQ_VERSION="$(sed -n 's/.*{vsn, *"\([^"]*\)"}.*/\1/p' src/reckon_evoq.app.src | head -n 1)"

echo
echo "==> reckon-evoq ${RECKON_EVOQ_VERSION}: building the package and docs"
rebar3 hex build

# reckon-evoq ships no NIF, so the docs build compiling the tree is the whole native
...
==> reckon-evoq ${RECKON_EVOQ_VERSION} is ready to release. Nothing was published.

To release it:
  git push origin v${RECKON_EVOQ_VERSION}
...
Afterwards, to check hex serves the tagged code rather than assuming it:
  scripts/is_hex_serving_what_git_says.sh ${RECKON_EVOQ_VERSION}
EOF
